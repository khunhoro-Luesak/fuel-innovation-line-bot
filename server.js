...
//  - เพิ่มเมนู Greeting ครบ 5 รายการ
...
// ===========================================================
app.get("/", (_req, res) => {
  res.send("🚀 Fuel Innovation LINE Bot (v4.9.6 – Smart FAQ + Saving Calculator + Annual Report) is running...");
});
...
async function handleEvent(event) {
  ...
  if (event.type === "message" && event.message.type === "text") {
    ...
    if (/คำนวณความประหยัด/i.test(msg)) {
      ...
      return replyText(event.replyToken, "🧮 กรุณาพิมพ์ราคาน้ำมันที่คุณซื้อจากหน้าปั๊ม (บาท/ลิตร):");
    }
    ...
    if (/เกี่ยวกับเรา|about/i.test(msg)) {
      logAction("about-card", { msg });
      return client.replyMessage(event.replyToken, buildAboutFlex());
    }
...
async function handleCalcFlow(userId, msg, replyToken) {
  ...
  if (isNaN(num) || num < 0) {
    return replyText(replyToken, "❗ กรุณาพิมพ์เป็นตัวเลขที่ถูกต้อง เช่น 33 หรือ 33.50");
  }

  // STEP 1: รับราคา/ลิตร
  if (state.step === 1) {
    calcState.set(userId, { step: 2, price: num });
    return replyText(replyToken, "📉 ส่วนลดที่ได้รับจาก Fuel Innovation (บาท/ลิตร):");
  }

  // STEP 2: รับส่วนลด/ลิตร
  if (state.step === 2) {
    calcState.set(userId, { step: 3, price: state.price, discount: num });
    return replyText(replyToken, "⛽ ปริมาณการใช้น้ำมันต่อวัน (ลิตร):");
  }
  ...
  // กรณีสถานะไม่คาดคิด
  calcState.delete(userId);
  return replyText(replyToken, "ขออภัยครับ เกิดข้อผิดพลาดในการคำนวณ กรุณาพิมพ์ “คำนวณความประหยัด” เพื่อเริ่มใหม่อีกครั้งครับ 🙏");
}
            ...
            contents: [
              {
                ...
                color: "#FBBF24",
                height: "sm",
                action: { type: "uri", label: "📄 ดูสเปคน้ำมัน", uri: SPEC_URL },
              },
              {
                type: "button",
                style: "primary",
                color: "#EF4444",
                height: "sm",
                action: { type: "uri", label: "🧾 ขอใบเสนอราคา", uri: QUOTE_URL },
              },
              {
                type: "button",
                style: "primary",
                color: "#8B5CF6",
                height: "sm",
                action: { type: "message", label: "🧮 คำนวณความประหยัด", text: "คำนวณความประหยัด" },
              },
              {
                type: "button",
                style: "primary",
                color: "#2563EB",
                height: "sm",
                action: { type: "message", label: "💬 ถาม–ตอบ", text: "ถาม–ตอบ" },
              },
              {
                type: "button",
                style: "primary",
                color: "#22C55E",
                height: "sm",
                action: { type: "message", label: "📞 ฝ่ายขาย", text: "ฝ่ายขาย" },
              },
            ],
          },
          {
      ...
      body: {
        ...
        contents: [
          ...
          },
          {
            type: "text",
            text:
              "บริษัท ฟูเอล อินโนเวชั่น จำกัด ก่อตั้งขึ้นเพื่อมุ่งพัฒนาและจัดจำหน่ายผลิตภัณฑ์น้ำมันอุตสาหกรรมทางเลือก (Alternative Industrial Diesel: AID) ที่มีประสิทธิภาพสูงและคุ้มค่ากว่าน้ำมันเชิงพาณิชย์ โดยอยู่ภายใต้ระบบควบคุมคุณภาพภายใน (Internal QC Standard) ทุกล็อตการผลิต\n\nเรามุ่งให้บริการแก่ลูกค้ากลุ่มอุตสาหกรรม ก่อสร้าง และขนส่งขนาดกลาง–ใหญ่ ที่ต้องการน้ำมันคุณภาพสูงสำหรับเครื่องจักร รถบรรทุก รถแม็คโคร และอุปกรณ์งานหนัก โดยมุ่งเน้นความคุ้มค่า ความทนทาน และความโปร่งใสในการให้บริการ\n\n🧩 ผลิตภัณฑ์หลัก:\nAlternative Industrial Diesel (AID) น้ำมันดีเซลอุตสาหกรรมทางเลือก ที่ผ่านกระบวนการกลั่นและกรองหลายขั้นตอน เพื่อให้ได้ค่าความหนืดและเชื้อน้ำมันที่เหมาะสมกับเครื่องจักรหนัก ประหยัดกว่า 4 บาทต่อลิตร โดยไม่กระทบต่อสมรรถนะของเครื่องยนต์\n\n💪 จุดแข็งของบริษัท:\n✅ ประหยัดสูงสุด 4 บาท/ลิตร\n✅ ระบบควบคุมคุณภาพภายใน (QC + COA)\n✅ จัดส่งตรงถึงไซต์งาน ปลอดภัย โปร่งใส\n✅ ให้คำปรึกษาโดยทีมผู้เชี่ยวชาญ\n✅ พร้อมลงนาม NDA เพื่อความมั่นใจของลูกค้าองค์กร\n\n🎯 กลุ่มลูกค้าเป้าหมาย:\n🏗️ บริษัทรับเหมาก่อสร้าง\n🚧 โครงการถนน สะพาน เขื่อน\n🚛 รถบรรทุกหนัก รถแม็คโคร รถเทรลเลอร์\n⛏️ ผู้ประกอบการเหมือง และโรงงานอุตสาหกรรมหนัก\n\n🌱 วิสัยทัศน์:\nมุ่งเป็นผู้นำด้านพลังงานอุตสาหกรรมทางเลือก เพื่อช่วยลดต้นทุนให้ผู้ประกอบการไทย พร้อมส่งเสริมการใช้พลังงานอย่างมีประสิทธิภาพและยั่งยืน",
            wrap: true,
            size: "sm",
            color: "#E5ECFF",
            margin: "sm",
...
// ===========================================================
app.listen(PORT, () => {
  console.log("🚀 Fuel Innovation LINE Bot running on port " + PORT + " (v4.9.6 – Smart FAQ + Saving Calculator + Annual Report)");
});
