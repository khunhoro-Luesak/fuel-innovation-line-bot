...
//  Version: 4.9.7 – Smart FAQ + Saving Calculator + Company Profile Link
//  - คงโครงสร้างและคอมเมนต์จาก v4.9.4 ทุกบรรทัด
//  - เพิ่มระบบคำนวณความประหยัด (3 ขั้นตอน) + แสดงผล รายวัน/เดือน/ปี
//  - เพิ่มเมนู Greeting ครบ 5+1 รายการ (รวม Company Profile)
//  - วางบล็อกคำนวณหลัง Smart Greeting Memory ตามที่ยืนยัน
//  - เปลี่ยน "เกี่ยวกับเรา" ให้ส่งลิงก์ Company Profile (PDF)
// ===========================================================
...
const COMPANY_PROFILE_URL = process.env.COMPANY_PROFILE_URL || ""; // ✅ NEW
...
app.get("/", (_req, res) => {
  res.send(
    "🚀 Fuel Innovation LINE Bot running on port " +
      PORT +
      " (v4.9.7 – Smart FAQ + Saving Calculator + Company Profile Link)"
  );
});
...
// ===========================================================
// 🔍 FAQ Search Function (v4.9.7 Precision Enhanced)
// ===========================================================
...
async function handleEvent(event) {
  ...
  if (event.type === "message" && event.message.type === "text") {
    ...
    if (/^สวัสดี/.test(msg)) {
      ...
      if (!canGreet(userId)) {
        return replyText(event.replyToken, `😊 ยินดีที่ได้คุยกับคุณ${name} อีกครั้งครับ`);
      }
      return replyText(
        event.replyToken,
        [
          ...
          ``,
          `พิมพ์ "เกี่ยวกับเรา" เพื่อดาวน์โหลด Company Profile (PDF)`,
    ...
    if (/(^ถาม$)|(^ถาม-ตอบ$)|(^ถาม–ตอบ$)/.test(msg)) {
      ...
      return replyText(
        ...
        [
          ...
          "",
          "• ทำไม Alternative Industrial Fuel – Type D1 ถึงราคาคุ้มค่า",
          "• เชื้อเพลิงมาจากไหน",
    ...
    // --- เกี่ยวกับเรา → ส่งลิงก์ Company Profile ---
    if (/เกี่ยวกับเรา|about/i.test(msg)) {
      logAction("about-link", { msg });
      const url = COMPANY_PROFILE_URL || SPEC_URL || QUOTE_URL || "https://google.com";
      return replyText(
        event.replyToken,
        [
          "🏢 Fuel Innovation – Company Profile",
          "",
          "ดาวน์โหลดเอกสารแนะนำบริษัทฉบับเต็ม (PDF) ได้ที่นี่ 👇",
          `📎 ${url}`,
          "",
          "หากต้องการดูสเปคเชื้อเพลิง Alternative Industrial Fuel – Type D1",
          'พิมพ์คำว่า "สเปคน้ำมัน" ได้เลยครับ 😊',
        ].join("\n")
      );
    }

    // --- ฝ่ายขาย (แยกจาก “ขายในปั๊ม”) ---
    if (/(^ฝ่ายขาย$)|(^ติดต่อ$)|(^ขาย$)|(^โทร$)|(^อยากซื้อ$)|(^ติดต่อฝ่ายขาย$)/.test(msg)) {
...
async function handleCalcFlow(userId, msg, replyToken) {
  ...
  if (state.step === 2) {
    calcState.set(userId, { step: 3, price: state.price, discount: num });
    return replyText(
      replyToken,
...
function buildWelcomeFlex() {
  const companyUrl = COMPANY_PROFILE_URL || SPEC_URL || QUOTE_URL || "https://google.com";
